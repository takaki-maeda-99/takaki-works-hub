...
namespace { // 無名名前空間: このファイル内でのみ有効な定数
  ...
  const float wheel_kinematics_coeffs[NUM_MOTORS][2] = {
    {cos45, -cos45}, {cos45, sin45}, {-cos45, sin45}, {-cos45, -sin45}
  };
}
...
void OmniRobotController::setManualVelocity(float vx, float vy, float omega) {
  if (vx != 0 || vy != 0 || omega != 0) {
    ...
    m_target_velocity_x = vx;
    m_target_velocity_y = vy;
...
OmniRobotController::State OmniRobotController::getCurrentState() const {
